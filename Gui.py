...
from popupwindow import Ui_myPopupWidget
...
class Gui(QtWidgets.QMainWindow,Ui_MainWindow):
    def __init__(self,parent=None):
        ...
        self.setWindowIcon(QtGui.QIcon("resources/Interpreter.png"))
        # 初始化弹出窗口
        self.popupwidget = QtWidgets.QWidget()
        self.ui_popupwindow = Ui_myPopupWidget()
        self.ui_popupwindow.setupUi(self.popupwidget)
        ...
        self.output = ""
        # 临时文件名
        self.filename = "temp.clike"
        # 正常运行颜色
        self.NORMAL_COLOR = QtGui.QColor(0,0,0)
        # 报错颜色
        self.ERROR_COLOR = QtGui.QColor(255,0,0)
        # 遇到报错
        self.is_error = False
        # 终止程序
        self.is_terminated = False
        # 检测用户输入是否完成的定时器
        self.userinput_timer = QTimer()
        self.userinput_timer.timeout.connect(self.onUserInput)
        # 输入完毕标志
        self.input_finish_flag = False
        # 默认代码
        self.default_code_file = "examples/helloworld.clike"
        if os.path.exists(self.default_code_file):
            self.filename = self.default_code_file
            with open(self.filename,'r') as f:
                self.content = f.read()
            self.CodeTextEdit.setText(self.content)
        # 按钮列表方便操作
        self.OtherButtons = [self.NewFilePBt,self.OpenFilePBt,self.SaveFilePBt,self.CleanFilePBt]
        
    def init_click_event(self):
        self.NewFilePBt.clicked.connect(self.NewFile)
        self.OpenFilePBt.clicked.connect(self.OpenFile)
        self.SaveFilePBt.clicked.connect(self.SaveFile)
        self.CleanFilePBt.clicked.connect(self.CleanFile)
        self.ExecuteFilePBt.clicked.connect(self.ExecuteFile)
        self.LexerPBt.clicked.connect(self.token_series)
        self.SyntaxTreePBt.clicked.connect(self.SyntaxTree)
    ...
    def OpenFile(self):
        ...
        filter_str = 'Clike Files (*.clike)'
        
        # 默认打开examples目录
        default_path = "examples/"
        if os.path.exists(default_path):
            filename, _ = QFileDialog.getOpenFileName(self, 'Open File', default_path,filter_str,options=options)
        else:
            filename, _ = QFileDialog.getOpenFileName(self, 'Open File', '', filter_str,options=options)
    ...
    def SaveFile(self):
        self.content = self.CodeTextEdit.toPlainText()
        options = QFileDialog.Options()
        options |= QFileDialog.ReadOnly
        
        # 设置文件过滤器，只显示后缀名为 .clike 的文件
        filter_str = 'Clike Files (*.clike)'
        # 设置默认文件名
        default_name = self.filename
        
        # 获取保存的文件名
        filename, _ = QFileDialog.getSaveFileName(self, 'Save File', default_name, filter_str, options=options)
        if filename:
            self.filename = filename  # 将选择的文件名保存到 self.filename
            # 在这里执行保存文件的操作，例如写入文件内容等
    ...
    def ExecuteFile(self):
        self.init()
        if self.ExecuteFilePBt.text() == "执行":
            self.ExecuteFilePBt.setText("停止")
            self.content = self.CodeTextEdit.toPlainText()
            for bt in self.OtherButtons:
                bt.setEnabled(False)
            probably_exception = self.interprete()
            if probably_exception:
                self.OutputTextEdit.setTextColor(self.ERROR_COLOR)
                if probably_exception[0] == 'ProgramError':
                    self.OutputTextEdit.setText("[Error]\n{}".format(probably_exception[1]))
                else:
                    self.OutputTextEdit.setText("[Error]\n解释器遇到了意外的错误：{}".format(probably_exception[1]))
                self.is_error = True
                self.ExecuteFilePBt.setText("执行")
                for bt in self.OtherButtons:
                    bt.setEnabled(True)
                    

        elif self.ExecuteFilePBt.text() == '停止':
            self.ExecuteFilePBt.setText("执行")
            self.OutputTextEdit.setText("[Interprete Terminated]\n")
            for bt in self.OtherButtons:
                bt.setEnabled(True) 
            try:
                self.semantic.terminate()
            except:
                pass
    ...
    def init(self):
        try:
            self.userinput_timer.stop()
            self.OutputTextEdit.cursorPositionChanged.disconnect(self.block_backspace)
            self.OutputTextEdit.removeEventFilter(self)
        except:
            pass
        self.content = None
        ...
        self.input_finish_flag = False
        self.OutputTextEdit.setText("")
        self.OutputTextEdit.setTextColor(QtGui.QColor(0,0,0))
        self.is_error = False
        self.is_terminated = False
    ...
    def QtMessageBox(self,state,str_1,str_2):
        ...
        self.Messagebox.setWindowIcon(QtGui.QIcon('resources/Interpreter.png'))
        self.Messagebox.show()
    
    # 处理输入请求
    def onUserInput(self,string=None):
        if not self.is_error and not self.is_terminated:
            if not self.userinput_timer.isActive():
                # 更新控制台文本，并确定不可删除的文本
                self.Update_Output_Content([string])
                self.non_deletable_text = string
                
                # 使光标聚焦到OutputTextEdit，准备输入
                ...
                self.OutputTextEdit.setFocus()
                
                # 绑定遇到指定位置则禁止删除的事件
                self.OutputTextEdit.cursorPositionChanged.connect(self.block_backspace)
                self.userinput_timer.start(1000)
                self.OutputTextEdit.installEventFilter(self)
                
            if self.input_finish_flag:
                # 复原变量并唤醒线程继续运行
                self.input_finish_flag = False
                self.non_deletable_text = None
                self.semantic.userInputValue = self.OutputTextEdit.toPlainText()[self.start_position:]
                self.output += self.OutputTextEdit.toPlainText()[self.start_position:] + '\n'
                self.userinput_timer.stop()
                self.OutputTextEdit.cursorPositionChanged.disconnect(self.block_backspace)
                self.OutputTextEdit.removeEventFilter(self)
                self.semantic.condition.wakeAll()
    
    # 重写了self.OutputTextEdit的按键事件类，主要针对回车以及Backspace键
    def eventFilter(self, obj, event):
        if obj is self.OutputTextEdit and event.type() == QtCore.QEvent.KeyPress:
            cursor = self.OutputTextEdit.textCursor()
            ...
            if event.key() in (Qt.Key_Return,Qt.Key_Enter):
                self.OutputTextEdit.setReadOnly(True)
                self.input_finish_flag = True
            # 删除或左方向键，根据光标位置条件来禁止删除，逻辑仿Spyder控制台
            if event.key() in (QtCore.Qt.Key_Backspace, QtCore.Qt.Key_Left):
                if position <= self.start_position:
                    return True
                if event.key() == Qt.Key_Backspace:
                    a = self.non_deletable_text
                    if self.non_deletable_text in selected_text:
                        before_text = self.OutputTextEdit.toPlainText()[:self.OutputTextEdit.toPlainText().find(selected_text)]
                        selected_start = len(before_text+self.non_deletable_text)
                        selected_end = len(before_text+selected_text)
                        cursor.setPosition(selected_start)
                        # QtGui.QTextCursor.KeepAnchor 表示保持选中状态
                        cursor.setPosition(selected_end,QtGui.QTextCursor.KeepAnchor)
                        self.OutputTextEdit.setTextCursor(cursor)
                        return True
        return super().eventFilter(obj, event)

    # 控制光标位置并阻止删除的槽函数
    def block_backspace(self):
        cursor = self.OutputTextEdit.textCursor()
        position = cursor.position()
        if position >= self.start_position:
            self.OutputTextEdit.setReadOnly(False)
    ...
    # 更新控制台文本
    def Update_Output_Content(self,data_list):
        if not self.is_error and not self.is_terminated:
            for data in data_list:
                self.output += data
            self.OutputTextEdit.setText("[Interpreting...]\n{}".format(self.output))
        
    # 处理异常信号
    def capture_exception(self,error_message):
        if error_message:
            self.is_error = True
            self.OutputTextEdit.setTextColor(self.ERROR_COLOR)
    ...
    # 处理完成信号
    def Execute_Finish_Event(self,finish_flag):
        if finish_flag and not self.is_error:
            SUCCESS_COLOR = QtGui.QColor(0,0,0)
            self.ExecuteFilePBt.setText("执行")
            self.OutputTextEdit.setTextColor(SUCCESS_COLOR)
            self.OutputTextEdit.setText("[Interprete Successfully]\n{}".format(self.output))
            for bt in self.OtherButtons:
                bt.setEnabled(True)
    ...
    def token_series(self):
        if not self.tokens:
            ...
            token_value = [t.value for t in self.tokens]
            display_string = ""
            for tk in token_value:
                display_string += tk + '\n'
               
            self.ui_popupwindow.label.setText("Tokens:")
            self.ui_popupwindow.textEdit.setText(display_string)
            self.popupwidget.setWindowTitle("词素序列")
            self.show_popupwidget()
    ...
    def SyntaxTree(self):
        if not self.SyntaxTree:
            ...
            self.ui_popupwindow.label.setText("SyntaxTree:")
            self.ui_popupwindow.textEdit.setText(self.SyntaxTree)
            self.popupwidget.setWindowTitle("语法树")
            self.show_popupwidget()
      
    # 弹出窗口函数
    def show_popupwidget(self):
        if self.popupwidget.isVisible():
            return self.popupwidget.close() and self.popupwidget.show()
        else:
            return self.popupwidget.show()
    ...
    def interprete(self):
        try:
            ...
            if not self.tokens:
                self.Execute_Finish_Event(1)
                return
            # 语法
            parser = Parser(self.tokens)
            parser.main()
        except ProgramError as pe:
            return ('ProgramError',pe)
        except Exception as e:
            return ('OtherError',e)
        
        self.SyntaxTree = parser.display_string
        ast = parser.tree.root
        
        # 语义+解释执行
        self.semantic = Semantic(ast)
        self.semantic.input_required.connect(self.onUserInput)
        self.semantic.dataReady.connect(self.Update_Output_Content)
        self.semantic.finish_interprete_flag.connect(self.Execute_Finish_Event)
        self.semantic.exception_signal.connect(self.capture_exception)
        self.semantic.start()
...
class Lexer(object):
    ...
    def move_pointer(self, step):
        self.content = self.content[step:]
    ...
    # 跳过空白字符
    def skip_blank(self):
        while self.content and self.is_blank(self.content[0]):
            self.move_pointer(1)
    ...
    def get_next_token(self):
        ...
        # 判断是否为关键字或标识符
        if self.content[0].isalpha() or self.content[0] == '_':
            while self.content[0].isalnum() or self.content[0] == '_':
                token_value += self.content[0]
                self.move_pointer(1)
            # 判断是否为关键字词组
        ...
        # 判断是否为数字常量
            ...
            while self.content[0].isdigit() or self.content[0] == '.':
                token_value += self.content[0]
                self.move_pointer(1)
            token_type = TOKEN_STYLE[2]

        # 判断是否为运算符
        elif self.is_operator(self.content[0]):
            token_value += self.content[0]
            self.move_pointer(1)
            
            # 如果是++或--或==
            if (token_value == '+' or token_value == '-' or token_value == '=') and (token_value == self.content[0]):
                token_value += self.content[0]
                token_type = DETAIL_TOKEN_STYLE[token_value]
                self.move_pointer(1)
            # 如果是>=或<=
            elif (token_value == '>' or token_value == '<') and self.content[0] == '=':
                token_value += self.content[0]
                token_type = DETAIL_TOKEN_STYLE[token_value]
                self.move_pointer(1)
            # 如果是!=
            elif token_value == '!' and self.content[0] == '=':
                token_value += self.content[0]
                token_type = DETAIL_TOKEN_STYLE[token_value]
                self.move_pointer(1)
            else:
        ...
        # 判断是否为分隔符
        elif self.is_delimiter(self.content[0]):
            token_value = ""
            if self.content[0] == '\"':
                token_value += self.content[0]
                self.move_pointer(1)
                try:
                    ...
                    raise ProgramError("无法找到右引号，请检查字符串定义是否正确 in line {}!".format(self.line_num))
                ...
                if '\"' in token_value[1:-1]:
                    print("字符串中暂不支持引号!")
                    raise ProgramError("字符串内部暂不支持嵌套引号 in line {}!".format(self.line_num))
            
            elif self.content[0] == '\'':
                raise ProgramError("字符串不支持单引号 in line {}!".format(self.line_num))
            else:
                token_value += self.content[0]
                token_type = DETAIL_TOKEN_STYLE[token_value]
                self.move_pointer(1)
        ...
        # 生成token
        token = Token(token_type, token_value, self.line_num)
...
class Parser(object):
    ...
    def display(self, node, depth=0):
        if not node:
            return
        self.display_string += '|   ' * depth
        self.display_string += '|-- '
        self.display_string += ('( self: %s %s, father: %s, left: %s, right: %s )' % (node.value, node.type, node.father.value if node.father else None,
...
class Semantic(QtCore.QThread):
    ...
    # 完成解释信号：用于标志解释完成
    finish_interprete_flag = pyqtSignal(int)
    ...
    def __init__(self, ast):
        ...
        # 捕获异常的槽函数
        self.exception_signal.connect(self._capture_exception)
        # 程序是否含有异常
        self.is_error = False 
        # 打印内容缓冲区
        self.buffer = []       
            ...
            if current_node.value == 'FunctionName':
                if current_node.first_son.value != 'main':
                    if current_node.first_son.value in inside_function:
                        self.exception_signal.emit("错误的函数命名：{}是内置函数!".format(current_node.first_son.value))
                        ...
                        if tmp_node_1.value == 'StateParameterList':
                            ...
                            while tmp_node_2:
                                ...
                                try:
                                    ...
                                    pass
                                self.symbol_dict[func_name][tmp_parameter_name] = {
    ...
    def _function_call(self, node=None):
        ...
        while current_node:
            ...
            if current_node.type == 'FUNCTION_NAME':
                ...
                if func_name == 'main':
                    self.exception_signal.emit("main函数不支持调用!")
                ...
                # 如果是自定义函数
                    ...
                    if func_name == self.current_function:
                        self.exception_signal.emit("暂不支持递归调用!")
                    self.function_call_flag[func_name] = True
                    try:
                        copy_symbol_table = copy.deepcopy(self.symbol_dict[func_name])
                    except RecursionError:
                        self.exception_signal.emit("递归层数已达到最大值:{},终止运行!".format(self.max_recursion_depth))
                ...
                while tmp_node:
                    ...
                    # 是某个变量
                        ...
                        if tmp_node.value not in self.symbol_dict[self.current_function]:
                            self.exception_signal.emit("变量{}尚未声明!".format(tmp_node.value))
                            
                        else:
                            if not self.symbol_dict[self.current_function][tmp_node.value]['init']:
                                self.exception_signal.emit("变量{}尚未初始化!".format(tmp_node.value))
                        ...
                        self.exception_signal.emit('parameter type is not supported yet!')
        ...
        if not is_inside_flag:
            ...
            if num_args < num_params:
                ...
                if all(value.get('init') == 0 for value in required_parameter_dict.values()):
                    self.exception_signal.emit("参数数量不一致：required {},but {} was given.".format(
                        num_params, num_args))
                ...
                if num_args >= num_not_inited_parameter:
                    if num_args == num_not_inited_parameter:
                        ...
                        if not_inited_parameter_type_list != parameter_type_list:
                            ...
                            self.exception_signal.emit("错误的函数参数，请检查类型和顺序是否一致!")
                        ...
                        for k, p in required_parameter_dict.items():
                            ...
                            if p['field_type'] == current_var_type:
                                ...
                                if required_parameter_dict[k]['init'] != 1:
                                    ...
                                    self.exception_signal.emit("错误的函数定义参数，请检查类型和顺序是否一致!")
        ...
        # 内置函数
            ...
            if parameter_cnt == 0:
                ...
                self.exception_signal.emit("参数数量不一致：required 1,but {} was given.".format(parameter_cnt))
    ...
    def printf_function(self,arg=None):
        ...
        if arg_type == 'IDENTIFIER':
            if not self.symbol_dict[self.current_function][arg]['init']:
                self.exception_signal.emit("变量{}尚未初始化!".format(arg))
        ...
        if arg_type == 'int' or arg_type == 'float':
            display_string += str(arg)
        elif arg_type == 'string':
            extract_string = self.extract_string(arg)
            display_string += extract_string
        elif arg_type == 'bool':
            display_string += str(arg) 
        else: # ERROR
            print("不受支持的参数类型 in function 'printf'")
            self.exception_signal.emit("不受支持的参数类型 in function 'printf'")
        self.buffer.append(display_string)
        
    def readInt_function(self,arg=None):
        display_string = None
        if arg == None:
            display_string = "请输入一个整型："
        else:
            arg_type = self.judge_type(arg)
            if arg_type == "IDENTIFIER":
                if not self.symbol_dict[self.current_function][arg]['init']:
            ...
            if arg_type == "int" or arg_type == "float":
                self.exception_signal.emit("错误的参数类型：required string but given {}.".format(arg_type))
                
            elif arg_type == "string":
                display_string = self.extract_string(arg)
            else: # ERROR
                self.exception_signal.emit("不受支持的参数类型 in function 'readInt'!")
        
        self.input_required.emit(display_string)
        
        self.mutex.lock()
        self.condition.wait(self.mutex) # 在此处等待主线程的信号
        return_value = self.userInputValue
        self.mutex.unlock()
        
        try:
            return int(eval(return_value))
        except:
            self.exception_signal.emit("不受支持的输入类型 in function 'readInt'!")
    ...
    def readFloat_function(self,arg=None):
        ...
        if arg == None:
            ...
            if arg_type == "int" or arg_type == "float":
                ...
                self.exception_signal.emit("不受支持的参数类型 in function 'readFloat'!")
        ...
        try:
            return float(eval(return_value))
        except:
            self.exception_signal.emit("不受支持的输入类型 in function 'readFloat'!")

    
    def readString_function(self,arg=None):
        display_string = None
        if arg == None:
            display_string = "请输入一个字符串型："
        else:
            arg_type = self.judge_type(arg)
            if arg_type == "IDENTIFIER":
                if not self.symbol_dict[self.current_function][arg]['init']:
            ...
            if arg_type == "int" or arg_type == "float":
                ...
                self.exception_signal.emit("不受支持的参数类型 in function 'readString'!")
        ...
        try:
            ...
            self.exception_signal.emit("不受支持的输入类型 in function 'readString'!")
    ...
    def _statement(self, node=None):
        ...
        while current_node:
            ...
            current_node = current_node.right
        # 将该变量存入符号表
        self.symbol_dict[self.current_function][variable_name] = {
    ...
    def _assignment(self, node=None):
        ...
        if current_node.type != 'IDENTIFIER':
            self.exception_signal.emit("赋值表达式右侧必须是一个标志符!")

        if current_node.right.value != 'Expression' and current_node.right.value != 'FunctionCall':
            self.exception_signal.emit("不支持的赋值类型:{}".format(current_node.right.value))
            
        # 首先判断该变量是否已进行过声明
        if current_node.value not in self.symbol_dict[self.current_function]:
            self.exception_signal.emit("变量{}尚未声明!".format(current_node.value))
        ...
        if current_node.right.value == 'Expression':
            ...
            if current_node.right.type == 'FunctionCall':
                ...
                if func_name not in self.symbol_dict and func_name not in inside_function:
                    self.exception_signal.emit("函数{}尚未声明，不可调用!".format(func_name))
            ...
            if current_node.value not in self.symbol_dict[self.current_function]:
                ...
                self.exception_signal.emit("变量{}未声明!".format(current_node.value))
                
            # 该变量的类型
            field_type = self.symbol_dict[self.current_function][current_node.value]['field_type']
            # 继续判断：赋值号左右两边类型是否一致？
            ...
            if expres['type'] == 'DIGIT_CONSTANT' or expres['type'] == 'STRING_CONSTANT' or expres['type'] == 'BOOL_CONSTANT':
                if field_constant_type != expres['type']:
                    self.exception_signal.emit("错误的赋值类型:{}!".format(expres['type']))
                    
                # 判断完成后，进行解释执行操作：
                if field_type == 'int':
                    # 通过四舍五入规则将右边强制转换为int并赋值
                    self.symbol_dict[self.current_function][current_node.value]['value'] = round(float(expres['value']))
                elif field_type == 'float':
                    # 直接转浮点即可
                    self.symbol_dict[self.current_function][current_node.value]['value'] = float(expres['value'])
                elif field_type == 'string':
                    self.symbol_dict[self.current_function][current_node.value]['value'] = expres['value']
                elif field_type == 'bool':
                    if expres['value'] == 'True':
                        self.symbol_dict[self.current_function][current_node.value]['value'] = True
                    elif expres['value'] == 'False':
                        self.symbol_dict[self.current_function][current_node.value]['value'] = False
                        
                flag = 1
                self.symbol_dict[self.current_function][current_node.value]['init'] = flag
            # 如果是变量赋值
            elif expres['type'] == 'VARIABLE':
                var = expres['value']
                var_field_type = self.symbol_dict[self.current_function][var]['field_type']
                var_constant_type = self.judge_constant_type(var_field_type)
                # 如果是函数体定义的变量
                if self.symbol_dict[self.current_function][var]['type'] == 'VARIABLE':
                    if self.symbol_dict[self.current_function][var]['init'] == 0:
                        self.exception_signal.emit("变量{}未初始化，无法赋值!".format(var))
                        
                    else:
                        # 如果变量值的常数类型不一致
                        if field_constant_type != var_constant_type:
                            self.exception_signal.emit("变量{}和{}值类型不一致，无法赋值！".format(
                                current_node.value, var))
                            
                        # 同理
                        if field_type == 'int':
                            self.symbol_dict[self.current_function][current_node.value]['value'] = self.symbol_dict[self.current_function][expres['value']]['value']
                        elif field_type == 'float':
                            self.symbol_dict[self.current_function][current_node.value]['value'] = self.symbol_dict[self.current_function][expres['value']]['value']
                        elif field_type == 'string':
                            self.symbol_dict[self.current_function][current_node.value]['value'] = self.symbol_dict[self.current_function][expres['value']]['value']
                        ...
                        self.symbol_dict[self.current_function][current_node.value]['init'] = flag
                # 如果是函数头定义的参数
                elif self.symbol_dict[self.current_function][var]['type'] == 'PARAMETER':
                    if field_constant_type != var_constant_type:
                        self.exception_signal.emit("变量{}和{}值类型不一致，无法赋值！".format(
                            current_node.value, var))
                ...
                pdb.set_trace()
                self.exception_signal.emit("不支持的赋值类型:{}".format(expres['type']))
    ...
    def _variable_initialization(self, node=None):
        ...
        while current_node:
            ...
            # 变量名
                ...
                if variable_name in [param for param,value in self.symbol_dict[self.current_function].items() if value.get('type') == 'PARAMETER']:
                    ...
                    self.exception_signal.emit("暂不支持在函数内声明与函数参数同名的变量：{} {}".format(variable_field_type,variable_name))
        ...
        if current_node.right.value != 'Expression' and current_node.right.value != 'FunctionCall':
            ...
            self.exception_signal.emit("变量初始化中的赋值错误!")
        ...
        if current_node.right.value == 'Expression':
            ...
            if expres['type'] == 'DIGIT_CONSTANT' or expres['type'] == 'STRING_CONSTANT' or expres['type'] == 'BOOL_CONSTANT':
                ...
                if field_type == 'int':
                    # 通过四舍五入规则将右边强制转换为int并赋值
                    self.symbol_dict[self.current_function][current_node.value]['value'] = round(expres['value'])
                elif field_type == 'float':
                    # 直接转浮点即可
                    self.symbol_dict[self.current_function][current_node.value]['value'] = float(expres['value'])
                elif field_type == 'string':
    ...
    def _control_for(self, node=None):
        ...
        while current_node:
            ...
            if len(self.buffer) >= 100:
                # 通知主线程数据已经准备好
                self.dataReady.emit(self.buffer[:100])
                # 移除数据
                    ...
                    if not current_node.first_son.right:
                        ...
                        if node_type == '_Variable':
                            if node_value not in self.symbol_dict[self.current_function] or self.symbol_dict[self.current_function][node_value]['init'] == 0:
                                self.exception_signal.emit("变量{}尚未声明或初始化!".format(node_value)) 
                        ...
                        func_name = current_node.first_son.value
                        if func_name not in self.symbol_dict and func_name not in inside_function:
                            self.exception_signal.emit("函数{}尚未声明，无法调用!".format(func_name))
            ...
            # for语句部分
                ...
                if current_node.extra_info == 'break':
                    self.jump_flag = False
                    current_node.extra_info = None
                    break
                elif current_node.extra_info == 'continue':
                    current_node.extra_infp = None
                    self.jump_flag = False
        ...
        while current_node:
            if current_node.value == 'IfControl':
                if current_node.first_son.value != 'Expression' and current_node.first_son.right.value != 'Sentence':
                    self.exception_signal.emit("if语句错误!")
                ...
                if current_node.first_son.value != 'Expression' and current_node.first_son.right.value != 'Sentence':
                    self.exception_signal.emit("elif语句错误!")
            ...
            if current_node.value == 'Expression':
                ...
                if not current_node.first_son.right:
                    ...
                    # 常数
                        ...
                        self.exception_signal.emit("不受支持的表达式类型{}!".format(node_type))
    ...
    def _return(self, node=None):
        ...
        if not current_node or not current_node.right:
            self.exception_signal.emit("return语句错误!")
        ...
        if current_node.value != 'return' or current_node.right.value != 'Expression':
            ...
            if expres['type'] == 'VARIABLE':
                if expres['value'] in self.symbol_dict[self.current_function] and self.symbol_dict[self.current_function][expres['value']]['init'] == 1:
                    ...
                    self.exception_signal.emit("返回变量{}不存在，或尚未初始化!".format(expres['value']))
                ...
                self.exception_signal.emit("return类型不支持!")
            self.jump_flag = True
        ...
        for item in statement:
            ...
            if item_type == '_Variable':
                if item_value not in self.symbol_dict[self.current_function]:
                    ...
                    self.exception_signal.emit("变量{}未声明!".format(item_value))
                    
                if self.symbol_dict[self.current_function][item_value]['init'] == 0:
                    print("变量{}未初始化!".format(item_value))
                    self.exception_signal.emit("变量{}未初始化!".format(item_value))
            ...
            elif item_type == 'STRING_CONSTANT':
                ...
                if item_value not in self.symbol_dict:
                    self.exception_signal.emit("函数{}尚未声明，无法调用!".format(item_value))
                else:
                ...
                # 双目运算符
                    ...
                    try:
                        ...
                        try:
                            if item_value == '+':
                                if left_operand[0] == '\"':
                                    left_operand = self.extract_string(left_operand)
                                if right_operand[0] == '\"':
                                    right_operand = self.extract_string(right_operand)
                                result = left_operand + right_operand
                                if isinstance(result,str):
                                    result = self.boxing_string(result)
                            elif item_value == '-':
                                result = left_operand - right_operand
                            elif item_value == '*':
                                result = left_operand * right_operand
                                ...
                                if left_type == 'Var':
                                    ...
                                    self.exception_signal.emit("赋值左值必须为变量!")
                        except TypeError as te:
                            self.exception_signal.emit(str(te))
                    ...
                    self.exception_signal.emit("operator {} not supported!".format(item_value))
    ...
    def _calculate_expression(self,node=None):
        ...
        if not node.extra_info:
            self.exception_signal.emit("不受支持的表达式形式!")
        reverse_polishexpression_statement = [tree.current if tree.root.type !='FunctionCall' else tree.root.first_son for tree in node.extra_info]
        ...
        if node.value in self.sentence_type:
            ...
            # 函数调用
                ...
                if func_name not in self.symbol_dict:
                    if func_name not in inside_function:
                        self.exception_signal.emit("函数{}尚未定义，无法调用!".format(func_name))
            ...
            # 控制语句
                ...
                if node.type == 'IfElseControl':
                    ...
                    self.exception_signal.emit('control type not supported!')
            # break语句
            ...
            # return语句
                ...
                self.exception_signal.emit('sentence type not supported yet！')
    ...
    def run(self):
        ...
        if not self.is_error:
            ...
            self.finish_interprete_flag.emit(1)
